...
public class ConnectionCreateCommand extends EditElementCommand {
	...
	public boolean canExecute() {
		...
		if (source != null && false == source instanceof HwComponent) {
			return false;
		}
		if (target != null && false == target instanceof HwComponent) {
			return false;
		}
		if (getSource() == null) {
		...
		return BlockDiagramBaseItemSemanticEditPolicy.getLinkConstraints()
				.canCreateConnection_4001(getSource(), getTarget());
	}
	...
	protected CommandResult doExecuteWithResult(IProgressMonitor monitor,
		...
		Connection newElement = BlockdiagramFactory.eINSTANCE
				.createConnection();
		getSource().getConection().add(newElement);
		newElement.setRef(getTarget());
	 ...
	 * @generated
	 */
	protected HwComponent getSource() {
		return (HwComponent) source;
	}

	/**
	 * @generated
	 */
	protected HwComponent getTarget() {
		return (HwComponent) target;
	}
